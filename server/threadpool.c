...
threadpool_t *threadpool_create(int _max_thread_num) {
   ...
   /* 为线程池动态申请内存,开辟空间*/ 
    int times=0;
    while(!(pool = malloc(sizeof(threadpool_t))))
        if(times<5)
            printf("thread_pool reallocing %d\n",times++);
        else return NULL;
    /* 初始化互斥量 */
    ...
    /* 为线程 id 数组开辟空间 */
    times=0;
    while(!(pool->threadId = calloc(_max_thread_num,sizeof(pthread_t))))
        if(times<5)
            printf("thread_pool->threadID reallocing %d\n",times++);
        else return NULL;
...
int threadpool_add_task(threadpool_t *pool,void *(*process)(void *arg),void *arg)
{
    ...
    int times=0;
    while(!(newWorker =  malloc(sizeof(CThread_worker))))
        if(times<5)
            printf("threadpool_add_task faile! %d\n",times++);
        else return -1;
